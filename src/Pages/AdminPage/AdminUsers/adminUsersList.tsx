...
export function AdminUsersList() {
  // States to manage users, loading, selected user, bookings, toast messages, and pagination
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        // Fetching users using currentPage for pagination
        const response = await fetch(
          `/api/admin/users?page=${currentPage}&limit=10&sortBy=lastName&sortDir=asc`,
          { credentials: "include" }
        );
  ...
  useEffect(() => {
    const fetchUserBookings = async () => {
      ...
      try {
        // Fetching bookings for the selected user
        const response = await fetch(
          `/api/admin/bookings/${selectedUser._id}?page=1&limit=5&sortBy=created_at&sortDir=desc`,
          { credentials: "include" }
        );
    ...
    fetchUserBookings();
    // Re-fetch bookings whenever selectedUser changes
  }, [selectedUser]);

  const handleUserClick = (user: User) => {
    // sets the selected user and clears previous toast messages
    setSelectedUser(user);
    setToastMessage(null);
  };
  // when closing the modal, reset selectedUser and bookings
  const closeModal = () => {
  ...
  const handleCancelBooking = async (bookingId: string) => {
    try {
      // Sending DELETE request to cancel the booking with bookingId
